...
import { ibanValidator, notblankValidator, NIFNIEValidator, nifnieValidator, uppercaseValidator, UppercaseValidator } from './mis-validadores'
  ...
  describe('notblankValidator', () => {
    describe('OK', () => {
      it('Una cadena', () => {
        ...
        const result = notblankValidator(valor)

        expect(result).toBeNull()
      });

      it('Con un 0', () => {
        expect(notblankValidator(new FormControl(0))).toBeNull()
      });
    ...
    describe('KO', () => {
      it('Solo blancos', () => {
        ...
        expect(result?.['notblank']).toBeDefined()
        expect(result?.['notblank']).toBe('Requerido, no debe estar en blanco')
      });
      it('cadena vacía', () => {
        const valor = new FormControl('')
